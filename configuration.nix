...
''; 
  ...
  environment.systemPackages = with pkgs; let
    osu = osu-lazer.overrideAttrs (old: {
      version = "2022.1228.0";
    });
  in [
    osu
  ...
  programs = {
    bash = {
      shellAliases = {
        grep = "grep --color";
        vpn = "screen sudo openvpn --config ~/.vpn/vpn.reports.mn/client.ovpn --auth-user-pass --auth-retry interact";
        techvpn = "screen sudo openvpn --config ~/.vpn/techvpn.reports.mn/client.ovpn --auth-user-pass --auth-retry interact";
        editnix = "sudo sh -c 'cd /etc/nixos; bash'";
      };
    };
    ssh = {
      startAgent = true;
